...
function initLiveChart() {
  ...
  liveChart = new Chart(ctx, {
    ...
    data: { labels: [], datasets: [
      { label: 'Sensor MAX31855', data: [], borderColor: '#E03C2E', borderWidth: 2, pointRadius: 3, fill: false, tension: 0.2 },
      { label: 'T Amb LM35', data: [], borderColor: '#3DE8A0', borderWidth: 1.5, borderDash: [6,4], pointRadius: 0, fill: false, tension: 0.3 }
    ]},
    options: {
...
function initHeatingCoolingCharts() {
  ...
  heatingChart = new Chart(ctxHeat, {
    type: 'scatter',
    data: { datasets: [
      { label: 'Datos reales', data: [], backgroundColor: '#E03C2E', pointRadius: 4 },
      { label: 'Ajuste exponencial', data: [], type: 'line', borderColor: '#2A86D4', borderWidth: 2, fill: false },
      { label: 'T Amb LM35', data: [], type: 'line', borderColor: '#3DE8A0', borderWidth: 1.5, borderDash: [6,4], pointRadius: 0, fill: false }
    ]},
...
function addDataPoint(t, temp, tAmb) {
  ...
  if (liveChart) {
    ...
    liveChart.data.datasets[1].data = ambData.map(v => (v !== null && !isNaN(v)) ? v : null);
...
document.getElementById('percentError').textContent = errorPromedio.toFixed(2) + ' %';
  ...
  if (heatingChart) {
    const heatAmb = ambData.slice(0, peakIdx + 1);
    ...
    heatingChart.data.datasets[2].data = heatTimes.map((t, i) => ({ x: t, y: (heatAmb[i] !== null && !isNaN(heatAmb[i])) ? heatAmb[i] : Tamb }));
    heatingChart.update();
  }
  if (coolingChart) {
    const coolAmb = ambData.slice(peakIdx);
    ...
    coolingChart.data.datasets[2].data = coolTimes.map((dt, i) => ({ x: dt, y: (coolAmb[i] !== null && !isNaN(coolAmb[i])) ? coolAmb[i] : Tamb }));
    coolingChart.update();
  }
